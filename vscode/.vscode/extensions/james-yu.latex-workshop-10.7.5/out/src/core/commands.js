...
async function view(mode) {
    ...
    return lw_1.lw.viewer.view(vscode.Uri.file(lw_1.lw.file.getPdfPath(pickedRootFile)), typeof mode === 'string' ? mode : undefined);
}
function refresh() {
...
function synctex() {
    ...
    if (lw_1.lw.root.subfiles.path && configuration.get('latex.rootFile.useSubFile')) {
        pdfUri = vscode.Uri.file(lw_1.lw.file.getPdfPath(lw_1.lw.root.subfiles.path));
    }
    else if (lw_1.lw.root.file.path !== undefined) {
        pdfUri = vscode.Uri.file(lw_1.lw.file.getPdfPath(lw_1.lw.root.file.path));
    }
